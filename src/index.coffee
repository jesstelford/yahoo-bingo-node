...
#     3. Remove that number from the mapping
###
...
client = io.connect 'ws://yahoobingo.herokuapp.com'
...
client.on 'card', (payload) ->
  ...
  colIndex = 0
  _(payload.slots).each (columnOfNumbers) ->

    rowIndex = 0
    _(columnOfNumbers).each (number) ->
      numbers[number] ?= []
      numbers[number].push colIndex
      numbers[number].push rowIndex
      numbers[number].push TL_BR_DIAGONAL_INDEX if colIndex is rowIndex # We're on the top-left to bottomr-right diagonal
      numbers[number].push TR_BL_DIAGONAL_INDEX if colIndex is BOARD_WIDTH - rowIndex # We're on the top-left to bottomr-right diagonal
...
client.on 'win', (message) ->
  ...
  tryAgain = false # We won, no need to try again

client.on 'lose', (message) ->
  console.log "LOSE!", message
  tryAgain = true # We want to try win another one
...
client.on 'disconnect', ->
  if tryAgain
    ...
    # TODO: Shut down the server somehow
